...
for densecpd_file in densecpd_files:
    if ".pred" in densecpd_file.name:
        ...
        current_file = pd.read_csv(densecpd_file, delimiter=" ", header=None, skipfooter=1)
        ...
        position_sequence = current_file.to_numpy()[:, 3::2]
        position_probabilities = current_file.to_numpy()[:, 4::2]

        if len(position_sequence) == 0:
            print(f"Error with PDB {pdb_code}")
            continue
        # position_sequence =
        assert len(position_sequence[0]) == len(position_probabilities[0])
        ...
        for curr_seq, curr_prob in zip(position_sequence, position_probabilities):
            current_encoding = np.zeros(20, dtype=float)
            for res, prob in zip(curr_seq, curr_prob):
                idx = res_encoder[res]
                current_encoding[idx] = prob
            current_sequence.append(current_encoding)
